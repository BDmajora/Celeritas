...
/**
 * The {@code alphaTest.<program> = off | <func> <ref>} directive (Iris's {@code AlphaTest}).
 * <p>
 * Iris runs on core profile, where there is no fixed-function alpha test, so it compiles the comparison into a
...
public final class ProgramAlphaTest {
    ...
    public static ProgramAlphaTest from(ShaderProperties properties, String programName) {
        ...
        if (function == null) {
            LOGGER.warn("[Iris] Unknown alpha test function '{}' in alphaTest.{}, ignoring it", parts[0], programName);
            return EMPTY;
        }
        ...
        if (parts.length > 1) {
            try {
                ...
                LOGGER.warn("[Iris] Malformed alpha test reference '{}' in alphaTest.{}, ignoring it",
                        parts[1], programName);
                return EMPTY;
            }
        } else if (function != GL11.GL_ALWAYS && function != GL11.GL_NEVER) {
            LOGGER.warn("[Iris] alphaTest.{} declares '{}' with no reference value, ignoring it", programName, raw);
            return EMPTY;
        }
