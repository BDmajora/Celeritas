...
import com.bdmajora.impetus.iris.gl.sampler.SamplerBinding;
import com.bdmajora.impetus.iris.gl.sampler.SamplerLimits;
...
/**
 * The samplers one program uses, each on a texture unit allocated for that program alone (Iris
 ...
 * therefore had to cap {@code IrisRenderTargets.MAX_COLOR_BUFFERS} at 16 (Iris allows 32) and park depthtex on 16-18,
 ...
 * anything close to all 49 names. This is exactly Iris's {@code nextUnit++} model.
 *
 * <h2>1.12.2 deviations from Iris</h2>
 * <ul>
 ...
 *   <li>Iris throws when a program runs out of units. This port logs and drops the sampler, matching the position
...
public final class ProgramSamplers {
     ...
     * {@link GlTextureUnits#bindTextureInRun(int, int, int)}. Iris does the same, and for the same reason: leaving the
    ...
    public static final class Builder {
        ...
        private Builder(int program, Set<Integer> reservedTextureUnits) {
            ...
            for (int unit : this.reservedTextureUnits) {
                if (unit >= this.maxTextureUnits) {
                    LOGGER.error("[Iris] Texture unit {} is reserved but this driver only has {}; samplers allocated "
        ...
        public void addExternalSampler(int textureUnit, String... names) {
            if (!this.reservedTextureUnits.contains(textureUnit)) {
                LOGGER.error("[Iris] Sampler(s) {} point at unit {}, which is not reserved; the allocator may reuse it",
            ...
            for (String name : names) {
                ...
                if (this.nextUnit >= this.maxTextureUnits) {
                    if (!this.exhaustedReported) {
                        LOGGER.error("[Iris] Program ran out of texture units at '{}' (driver reports {}); this and "
