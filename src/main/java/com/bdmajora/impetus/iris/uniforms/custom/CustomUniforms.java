...
public final class CustomUniforms {
        ...
        for (Variable variable : this.variables) {
            ...
            try {
                ...
                if (!variable.warned) {
                    ...
                    LOGGER.warn("[Iris] Custom uniform '{}' failed to evaluate: {}", variable.name, e.toString());
                }
            }
        }
    }
    ...
    public static final class Builder {
        ...
        public void addVariable(String typeToken, String name, String expression, boolean isUniform) {
            ...
            if (type == null) {
                LOGGER.warn("[Iris] Unknown custom uniform type '{}' for '{}', ignoring", typeToken, name);
                return;
            }
            if (this.pending.containsKey(name)) {
                LOGGER.warn("[Iris] Duplicate custom uniform/variable '{}', keeping the first definition", name);
                return;
            }
        ...
        public CustomUniforms build() {
            ...
            for (PendingVariable p : this.pending.values()) {
                try {
                    ...
                    LOGGER.warn("[Iris] Failed to parse custom {} '{}' = '{}': {}",
                            p.isUniform ? "uniform" : "variable", p.name, p.expression, e.getMessage());
                }
            }

            if (!variables.isEmpty()) {
                LOGGER.info("[Iris] Loaded {} custom uniform(s)/variable(s)", variables.size());
            }
