 ...
 * <p>
 * The meshing pipeline calls these when Umbra is active (Task #6 wiring): {@code mc_Entity} = (block id, block meta).
 * <p>
 * There is deliberately no {@code mc_midTexCoord} helper here. That attribute is the centre of the texture region
 * mapped to a <em>quad</em> (the mean of its four vertex UVs, as in Umbra's {@code XHFPTerrainVertex}), so it cannot
 ...
 * computation lives at the two meshing sites (see {@code VintageBlockRenderer.populateUmbraVertexData}).
 */
