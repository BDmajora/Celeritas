 ...
 */
public final class UmbraDebugDump {
    ...
    private static final int MAX_DUMP_FILES =
            Integer.getInteger("impetus.umbra.debugDumpMaxFiles", 256);
    private static final long MAX_DUMP_BYTES =
            Long.getLong("impetus.umbra.debugDumpMaxBytes", 64L * MIB);
    /**
    ...
    public static void dumpText(String name, String content) {
        try {
            ...
            LOGGER.error("[Umbra] Failed to dump text {}", name, e);
        }
    }
    ...
    private static boolean ensureDebugDir(File dir) {
        ...
        if (dir.exists()) {
            LOGGER.warn("[Umbra] Could not create debug dump directory because {} already exists", dir);
            return false;
        }
        if (!dir.mkdirs()) {
            LOGGER.warn("[Umbra] Could not create debug dump directory {}", dir);
            return false;
        }
        return true;
    }
    ...
    private static void warnBudgetOnce(String name, String format, Object... args) {
        ...
        LOGGER.warn("[Umbra] impetus_debug budget reached; skipping dump " + name + ": " + format, args);
    }
