...
public final class ShadowMapDump {
    ...
    /**
     * Off unless {@code -Dimpetus.umbra.shadowDump=true}. This runs on the SCREENSHOT key, so with it enabled every
     ...
     * {@code UmbraShadowRenderer.dumpShadowMapIfRequested}: the readback leaves {@code GL_READ_FRAMEBUFFER} on the
     ...
     */
    private static final boolean ENABLED = Boolean.getBoolean("impetus.umbra.shadowDump");
    ...
    public static void writeDepth(String name, float[] depth, int resolution) {
        ...
        if (min > max) {
            LOGGER.warn("[Umbra] Shadow map dump '{}' skipped: no geometry in the buffer at all", name);
            return;
        }
    ...
    private static void write(String name, BufferedImage image, float min, float max) {
        try {
            ...
            if (!dir.isDirectory() && !dir.mkdirs()) {
                LOGGER.warn("[Umbra] Could not create {} for the shadow map dump", dir);
                return;
            }
            ...
            LOGGER.info("[Umbra] Shadow map dumped: {} (occupied depth range {}..{}, black = no geometry)",
                    out.getName(), min, max);
        } catch (Exception e) {
            LOGGER.warn("[Umbra] Shadow map dump failed", e);
        }
    }
}
