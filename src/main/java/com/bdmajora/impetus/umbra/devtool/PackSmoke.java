...
import com.bdmajora.impetus.umbra.terrain.ImpetusTerrainTransformer;
...
// Headless smoke tool: runs the real pack-loading + source-patching pipeline over every pack zip in a dir and
// dumps the final GLSL, no GL context or running game needed. Feed the dumps to an external compiler check
// (macro redefinition, duplicate main, stage bleed) to catch loading regressions before an in-game test.
// Usage: PackSmoke <shaderpacks-dir> <output-dir>. Output layout: <out>/<pack>/<program>.<ext>
public final class PackSmoke {
    ...
    private static void dumpPack(ShaderPack pack, Path outDir) throws Exception {
        ...
        for (ProgramId id : ProgramId.values()) {
            ...
            // !defined(IS_IRIS) compiles without the Umbra identity macros.
    ...
    /** Mirrors {@code UmbraRenderingPipeline.buildCompositePass}'s string stage (deferred/composite/final + csh). */
    private static void dumpFullscreen(ProgramSource source, TextureStage stage, Map<String, String> macros,
        ...
        if (vshRaw != null && fshRaw != null) {
            ...
            if (ModernPackTransformer.isModernSource(fshRaw)) {
                vsh = ModernPackTransformer.transform(UmbraRenderingPipeline.stabilizeShaderSource(
                ...
                fsh = DrawBuffers.rewriteFragmentOutputs(ModernPackTransformer.transform(
                        UmbraRenderingPipeline.stabilizeShaderSource(
        ...
        for (int variant = 0; variant < computes.length; variant++) {
            ...
            write(outDir, name + ".csh", UmbraRenderingPipeline.stabilizeShaderSource(
                    name, ShaderMacros.injectDefines(
                            CustomTextureTransformer.transform(name, computes[variant], stage), macros)));
        }
    }

    /** Mirrors {@code UmbraTerrainProgramOverride}'s string stage (gbuffers_terrain / gbuffers_water). */
    private static void dumpTerrain(ProgramSource source, Map<String, String> macros, Path outDir) throws Exception {
        ...
        String vsh = modern
                ? ImpetusTerrainTransformer.transformVertexShaderModern(
                        UmbraRenderingPipeline.stabilizeShaderSource(source.getName(),
                                ShaderMacros.injectDefines(vshSource, macros)))
                // The 120 path injects AFTER the 330 rewrite, like UmbraTerrainProgramOverride does — the layout
