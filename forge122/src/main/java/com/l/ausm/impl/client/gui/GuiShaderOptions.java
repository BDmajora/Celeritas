...
public class GuiShaderOptions extends GuiScreen {
    ...
    private List<ShaderScreenEntry> visibleEntries = java.util.Collections.emptyList();
    private List<SidebarItem> sidebarItems = java.util.Collections.emptyList();
    ...
    private List<String> hoveredCommentTitle = java.util.Collections.emptyList();
    private List<String> hoveredCommentBody = java.util.Collections.emptyList();
    private int page;
    ...
    private void addOptionButtons(ShaderProperties properties) {
        if (searchActive()) {
            ...
            this.visibleEntries = screen == null ? java.util.Collections.emptyList() : screen.entries().stream()
    ...
    private String resetProfile(ShaderProperties properties) {
        ...
        String matchingDefault = matchingProfile(properties, java.util.Collections.emptyMap());
    ...
    private List<String> findScreenPath(ShaderProperties properties, String currentScreen, String targetScreen, Set<String> visited) {
        ...
        if (currentScreen.equals(targetScreen)) {
            return new ArrayList<>(com.l.ausm.impl.util.J8.list(currentScreen));
        }
    ...
    private List<SidebarItem> sidebarItems(ShaderProperties properties) {
        ...
        for (String screen : properties.screens().keySet()) {
            ...
            if (expandedSidebarScreens.contains(screen)) {
                addSidebarChildren(properties, screen, 1, items, new HashSet<>(com.l.ausm.impl.util.J8.set(screen)), false);
            }
        }
        return items;
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        lastMouseX = mouseX;
        lastMouseY = mouseY;
        hoveredCommentTitle = java.util.Collections.emptyList();
        hoveredCommentBody = java.util.Collections.emptyList();
        if (previewHidden) {
    ...
    private void setHoveredComment(GuiButton button, List<String> tooltip) {
        if (button instanceof GuiShaderOptionSlider) {
            GuiShaderOptionSlider slider = (GuiShaderOptionSlider) button;
            hoveredCommentTitle = com.l.ausm.impl.util.J8.list(optionName(slider.option.name()));
        } else if (button instanceof GuiShaderOptionDropdown) {
            GuiShaderOptionDropdown dropdown = (GuiShaderOptionDropdown) button;
            hoveredCommentTitle = com.l.ausm.impl.util.J8.list(optionName(dropdown.option.name()));
        } else if (button instanceof GuiShaderProfileDropdown) {
            hoveredCommentTitle = com.l.ausm.impl.util.J8.list("Profile");
        } else if (button.id >= OPTION_BASE_ID) {
            int index = button.id - OPTION_BASE_ID;
            if (index >= 0 && index < visibleEntries.size()) {
                hoveredCommentTitle = com.l.ausm.impl.util.J8.list(label(visibleEntries.get(index).name()));
            }
        } else {
            hoveredCommentTitle = com.l.ausm.impl.util.J8.list(button.displayString);
        }
    ...
    private List<GuiButton> focusableButtons() {
        return buttonList.stream().filter(this::isFocusable).collect(java.util.stream.Collectors.toList());
    }
