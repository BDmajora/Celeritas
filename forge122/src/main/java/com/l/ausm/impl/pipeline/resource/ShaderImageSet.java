...
public final class ShaderImageSet {
    ...
    private ShaderImageSet(List<ShaderImageDirective> images) {
        this.images = java.util.Collections.unmodifiableList(new java.util.ArrayList<>(images));
    }

    public static ShaderImageSet empty() {
        return new ShaderImageSet(java.util.Collections.emptyList());
    }
    ...
    public void bind(ShaderProgram program) {
        for (LoadedImage image : loadedImages) {
            ...
            if (directive.samplerName() != null && !directive.samplerName().trim().isEmpty()) {
