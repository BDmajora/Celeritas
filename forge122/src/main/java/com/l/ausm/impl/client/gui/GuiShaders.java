...
// Drag-and-drop pack import used LWJGL3 GLFW callbacks; disabled on Celeritas' LWJGL2 backend.
...
public class GuiShaders extends GuiScreen {
    ...
    // GLFW drop-callback fields removed (LWJGL2 backend has no GLFW window drop API).
    ...
    private List<GuiButton> focusableButtons() {
        return this.buttonList.stream().filter(this::isFocusable).collect(java.util.stream.Collectors.toList());
    }
    ...
    private void installDropCallback() {
        // No-op on LWJGL2: GLFW window drop callbacks are unavailable. Users import packs by placing them in the
        // shaderpacks/ folder and pressing Refresh.
    }

    private void restoreDropCallback() {
        // No-op on LWJGL2 (see installDropCallback).
    }
