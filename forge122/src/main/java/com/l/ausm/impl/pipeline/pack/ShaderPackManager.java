...
public class ShaderPackManager implements ShaderPackController {
    ...
    private Map<String, String> currentOptionOverrides = java.util.Collections.emptyMap();
    ...
    public boolean loadPack(String packName) {
        if (isOffPack(packName)) {
            ...
            currentOptionOverrides = java.util.Collections.emptyMap();
    ...
    public void loadSavedConfiguration() {
        ...
        shadersEnabled = false;
        currentPack = NoneShaderPack.INSTANCE;
        currentOptionOverrides = isOffPack(selectedPackName) ? java.util.Collections.emptyMap() : loadOptionOverrides(selectedPackName);
    ...
    private void setPack(ShaderPack newPack) {
        ...
        this.currentOptionOverrides = isInternalPack(newPack) ? java.util.Collections.emptyMap() : loadOptionOverrides(newPack.getName());
    ...
    private ShaderProperties getShaderProperties(String packName, Map<String, String> overrides, int dimensionId) {
        if (isOffPack(packName)) {
            return ShaderProperties.load(NoneShaderPack.INSTANCE, java.util.Collections.emptyMap());
        }

        Map<String, String> safeOverrides = overrides == null || overrides.isEmpty() ? java.util.Collections.emptyMap() : java.util.Collections.unmodifiableMap(new java.util.LinkedHashMap<>(overrides));
    ...
    private String compiledPipelineCacheKey(String packName, Map<String, String> optionOverrides, int dimensionId) {
        Map<String, String> safeOverrides = optionOverrides == null || optionOverrides.isEmpty()
                ...
                : java.util.Collections.unmodifiableMap(new java.util.LinkedHashMap<>(optionOverrides));
        return packName
                + "|world" + dimensionId
    ...
    private void copyDirectory(Path source, Path target) throws IOException {
        try (Stream<Path> stream = Files.walk(source)) {
            for (Path path : stream.sorted().collect(java.util.stream.Collectors.toList())) {
    ...
    private static boolean isOffPack(String packName) {
        return packName == null || packName.trim().isEmpty() || packName.equalsIgnoreCase(OFF_PACK_NAME);
    }
