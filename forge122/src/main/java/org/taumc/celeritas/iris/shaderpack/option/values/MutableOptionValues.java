package org.taumc.celeritas.iris.shaderpack.option.values;

import org.taumc.celeritas.iris.shaderpack.OptionalBoolean;
...
/**
 * A mutable snapshot of option values. Values equal to the pack default are dropped so that only genuine overrides are
 * stored (this is what gets persisted to {@code <pack>.txt}). Ported from Iris; guava {@code ImmutableMap} replaced
 * with plain {@link HashMap}s.
 */
