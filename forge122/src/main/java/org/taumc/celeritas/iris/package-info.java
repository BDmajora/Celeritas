/**
 * Iris-style OptiFine shader-pack support for Celeritas on Minecraft 1.12.2.
 *
 ...
 * rendering behavior changes yet</b>: with no pack selected (or on any load failure) Celeritas renders exactly as it
 * does without this module. That zero-cost-when-disabled property is a hard requirement of the port.
 *
 * <h3>Implemented</h3>
 * <ul>
 *   <li>{@link org.taumc.celeritas.iris.shaderpack.ShaderPackLoader} — folder/zip reading (java.nio/zip only).</li>
 *   <li>{@link org.taumc.celeritas.iris.shaderpack.ShaderPack} / {@code ProgramSet} / {@code ProgramSource} — the
 ...
 *   <li>{@link org.taumc.celeritas.iris.shaderpack.ShaderProperties} — {@code shaders.properties} parser.</li>
 *   <li>{@link org.taumc.celeritas.iris.shaderpack.include.IncludeProcessor} — recursive {@code #include} flattening
 *       with cycle detection and OptiFine relative/absolute path rules.</li>
 *   <li>{@link org.taumc.celeritas.iris.shaderpack.preprocessor.GlslPreprocessor} — {@code #version} detection and
 *       {@code #define} injection.</li>
 *   <li>{@link org.taumc.celeritas.iris.config.IrisConfig} — {@code optionsshaders.txt} read/write.</li>
 *   <li>{@link org.taumc.celeritas.iris.Iris} — orchestrator wired into the {@code @Mod} init.</li>
 * </ul>
 *
 * <h3>Deliberately deferred to later phases</h3>
 * <ul>
 *   <li>GL program compilation/linking via the Celeritas {@code org.taumc.celeritas.lwjgl} abstraction (the
 *       {@code LWJGLService} interface must first be extended with the texture/FBO/draw entry points Iris needs).</li>
