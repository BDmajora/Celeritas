 ...
 * each program is compiled <em>untransformed</em> and simply bound around its vanilla render section; only Embeddium
 * terrain (custom vertex format) and the fullscreen passes (core-profile quad) need GLSL transformation.
 * <p>
 * OptiFine's fallback chain is honored via {@link org.taumc.celeritas.iris.shaderpack.ProgramSet#get(ProgramId)}, and
