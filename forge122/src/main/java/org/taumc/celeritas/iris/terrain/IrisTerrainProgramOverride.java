...
/**
 * Builds the shader pack's {@code gbuffers_terrain}/{@code gbuffers_water}, transformed to Embeddium's vertex format
 * via {@link EmbeddiumTerrainTransformer} and wrapped in {@link IrisTerrainShaderInterface}. Iris-native: the pack
 * source comes from Celeritas' own {@link ShaderPack}/{@link Iris} model. Returned to {@code MixinShaderChunkRenderer};
 * on any failure returns {@code null} so Embeddium's default terrain shader is used and rendering never crashes.
 * <p>
...
public final class IrisTerrainProgramOverride {
    private static final Logger LOGGER = LogManager.getLogger("Celeritas/IrisTerrain");
     ...
     * manage (they are handed out during the shadow pass and never stored in Embeddium's per-renderer map), so they
    ...
    private static GlProgram<ChunkShaderInterface> build(ShaderPack pack, ChunkShaderOptions options, ProgramId programId) {
        ...
        try {
            ...
            var builder = GlProgram.builder("celeritas:iris_terrain");
            ...
            LOGGER.error("[Iris] Failed to build terrain override; using Embeddium default", e);
            return null;
        } finally {
